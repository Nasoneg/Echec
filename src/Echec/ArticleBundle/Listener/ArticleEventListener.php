...
class ArticleEventListener extends \ED\BlogBundle\Listener\ArticleEventListener
{
    protected $doctrine;
    protected $session;
    ...
    public function postSave(ArticleAdministrationEvent $event)
    {
        ...
        if($article)
        {
            ...
            $this->session->getFlashBag()->add('success', 'Article mis à jour avec succès.');
        }
    }
    ...
    public function postCreated(TaxonomyArrayEvent $event)
    {
        ...
        $this->session->getFlashBag()->add('success', 'Article créé avec succès.');
    }
