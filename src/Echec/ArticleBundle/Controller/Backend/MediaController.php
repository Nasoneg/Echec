...
class MediaController extends \ED\BlogBundle\Controller\Backend\MediaController
{
    /**
    ...
    public function uploadExcerptAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            return new JsonResponse(array('success' => 'true', 'id' => $media->getId(), 'href' => $this->generateUrl('ed_blog_admin_media_delete', array(
                'id' => $media->getId())),
                'media' => $this->renderView('EchecArticleBundle:Media:image.html.twig', array('media' => $media))));
        }
    ...
    public function editInfoAction(Request $request, Media $media)
    {
        ...
        return $this->render("EchecArticleBundle:Media:editInfoForm.html.twig", array(
            "id" => $media->getId(),
