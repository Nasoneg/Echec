...
class TagController extends \ED\BlogBundle\Controller\Backend\TagController
{
    /**
        ...
        if($request->isXmlHttpRequest())
        {
            ...
            if(!$title)
            {
                return new JsonResponse(array(
                    'success' => false,
                    'message' => "Le titre doit être défini"
                ));
            }
    ...
    public function listAction(Request $request)
    {
        ...
        $response = $paginator->paginate(
            ...
            'EchecArticleBundle:Taxonomy/Tag:list',
        ...
        if($request->isMethod('post'))
        {
            ...
            if($tag->getTerm()->getTitle())
            {
                ...
                if(!empty($existingTag))
                {
                    ...
                    $form->addError( new FormError("Désolé, un tag similaire \"" . $existingTag->getTerm()->getTitle()  . "\" existe déjà.") );
                }
            }
            ...
            if($form->isValid())
            {
                ...
                $this->get('session')->getFlashBag()->add('success', 'Tag créé avec succès.');
    ...
    public function editAction(Request $request, $slug)
    {
        ...
        if(!$taxonomy)
            throw new NotFoundHttpException("Désolé, la ressource n\'a pas pu être trouvée.");
        ...
        if($request->isMethod('post'))
        {
            ...
            if($tag->getTerm()->getTitle())
            {
                ...
                if(!empty($existingTags))
                {
                    ...
                    foreach($existingTags as $existing)
                    {
                        ...
                        {
                            $form->addError( new FormError("Désolé, un tag similaire \"" . $existing->getTerm()->getTitle()  . "\" existe déjà.") );
                        }
                    }
                }
            }
            ...
            if($form->isValid())
            {
                ...
                $this->get('session')->getFlashBag()->add('success', 'Tag modifié avec succès.');
                return $this->redirectToRoute('ed_blog_tag_list');
            }
        }
    ...
    public function deleteAction($slug)
    {
        ...
        $this->get('session')->getFlashBag()->add('success', 'Tag supprimé avec succès.');
        return $this->redirectToRoute('ed_blog_tag_list');
    }
    ...
    public function topTagsAction($number)
    {
          ...
          return $this->render("EchecArticleBundle:Frontend/Blog:blog_sidebar_tags.html.twig", array('topNtags' => $topNtags));
    }
}